.class public Landroidx/recyclerview/widget/DiffUtil;
.super Ljava/lang/Object;
.source "DiffUtil.java"


# annotations
.annotation system Ldalvik/annotation/MemberClasses;
    value = {
        Landroidx/recyclerview/widget/DiffUtil$CenteredArray;,
        Landroidx/recyclerview/widget/DiffUtil$PostponedUpdate;,
        Landroidx/recyclerview/widget/DiffUtil$DiffResult;,
        Landroidx/recyclerview/widget/DiffUtil$Range;,
        Landroidx/recyclerview/widget/DiffUtil$Snake;,
        Landroidx/recyclerview/widget/DiffUtil$Diagonal;,
        Landroidx/recyclerview/widget/DiffUtil$ItemCallback;,
        Landroidx/recyclerview/widget/DiffUtil$Callback;
    }
.end annotation


# static fields
.field private static final DIAGONAL_COMPARATOR:Ljava/util/Comparator;
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "Ljava/util/Comparator<",
            "Landroidx/recyclerview/widget/DiffUtil$Diagonal;",
            ">;"
        }
    .end annotation
.end field


# direct methods
.method public static constructor <clinit>()V
    .locals 1

    new-instance v0, Landroidx/recyclerview/widget/DiffUtil$1;

    invoke-direct {v0}, Landroidx/recyclerview/widget/DiffUtil$1;-><init>()V

    sput-object v0, Landroidx/recyclerview/widget/DiffUtil;->DIAGONAL_COMPARATOR:Ljava/util/Comparator;

    return-void
.end method

.method private constructor <init>()V
    .locals 0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    return-void
.end method

.method private static backward(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;I)Landroidx/recyclerview/widget/DiffUtil$Snake;
    .locals 11

    .line 1
    iget v0, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    iget v1, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    sub-int v2, v0, v1

    .line 2
    iget v3, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iget v4, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    sub-int v5, v3, v4

    sub-int/2addr v2, v5

    .line 3
    rem-int/lit8 v2, v2, 0x2

    const/4 v5, 0x1

    if-nez v2, :cond_0

    const/4 v2, 0x1

    goto :goto_0

    :cond_0
    const/4 v2, 0x0

    :goto_0
    sub-int/2addr v0, v1

    sub-int/2addr v3, v4

    sub-int/2addr v0, v3

    neg-int v1, p4

    move v3, v1

    :goto_1
    if-gt v3, p4, :cond_7

    if-eq v3, v1, :cond_2

    if-eq v3, p4, :cond_1

    add-int/lit8 v4, v3, 0x1

    .line 4
    invoke-virtual {p3, v4}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v4

    add-int/lit8 v6, v3, -0x1

    invoke-virtual {p3, v6}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v6

    if-ge v4, v6, :cond_1

    goto :goto_2

    :cond_1
    add-int/lit8 v4, v3, -0x1

    .line 5
    invoke-virtual {p3, v4}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v4

    add-int/lit8 v6, v4, -0x1

    goto :goto_3

    :cond_2
    :goto_2
    add-int/lit8 v4, v3, 0x1

    .line 6
    invoke-virtual {p3, v4}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v4

    move v6, v4

    .line 7
    :goto_3
    iget v7, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iget v8, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    sub-int/2addr v8, v6

    sub-int/2addr v8, v3

    sub-int/2addr v7, v8

    if-eqz p4, :cond_4

    if-eq v6, v4, :cond_3

    goto :goto_4

    :cond_3
    add-int/lit8 v8, v7, 0x1

    goto :goto_5

    :cond_4
    :goto_4
    move v8, v7

    .line 8
    :goto_5
    iget v9, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    if-le v6, v9, :cond_5

    iget v9, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    if-le v7, v9, :cond_5

    add-int/lit8 v9, v6, -0x1

    add-int/lit8 v10, v7, -0x1

    .line 9
    invoke-virtual {p1, v9, v10}, Landroidx/recyclerview/widget/DiffUtil$Callback;->areItemsTheSame(II)Z

    move-result v9

    if-eqz v9, :cond_5

    add-int/lit8 v6, v6, -0x1

    add-int/lit8 v7, v7, -0x1

    goto :goto_5

    .line 10
    :cond_5
    invoke-virtual {p3, v3, v6}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->set(II)V

    if-eqz v2, :cond_6

    sub-int v9, v0, v3

    if-lt v9, v1, :cond_6

    if-gt v9, p4, :cond_6

    .line 11
    invoke-virtual {p2, v9}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v9

    if-lt v9, v6, :cond_6

    .line 12
    new-instance p0, Landroidx/recyclerview/widget/DiffUtil$Snake;

    invoke-direct {p0}, Landroidx/recyclerview/widget/DiffUtil$Snake;-><init>()V

    .line 13
    iput v6, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->startX:I

    .line 14
    iput v7, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->startY:I

    .line 15
    iput v4, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->endX:I

    .line 16
    iput v8, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->endY:I

    .line 17
    iput-boolean v5, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->reverse:Z

    return-object p0

    :cond_6
    add-int/lit8 v3, v3, 0x2

    goto :goto_1

    :cond_7
    const/4 p0, 0x0

    return-object p0
.end method

.method public static calculateDiff(Landroidx/recyclerview/widget/DiffUtil$Callback;)Landroidx/recyclerview/widget/DiffUtil$DiffResult;
    .locals 1

    const/4 v0, 0x1

    .line 1
    invoke-static {p0, v0}, Landroidx/recyclerview/widget/DiffUtil;->calculateDiff(Landroidx/recyclerview/widget/DiffUtil$Callback;Z)Landroidx/recyclerview/widget/DiffUtil$DiffResult;

    move-result-object p0

    return-object p0
.end method

.method public static calculateDiff(Landroidx/recyclerview/widget/DiffUtil$Callback;Z)Landroidx/recyclerview/widget/DiffUtil$DiffResult;
    .locals 14

    .line 2
    invoke-virtual {p0}, Landroidx/recyclerview/widget/DiffUtil$Callback;->getOldListSize()I

    move-result v0

    .line 3
    invoke-virtual {p0}, Landroidx/recyclerview/widget/DiffUtil$Callback;->getNewListSize()I

    move-result v1

    .line 4
    new-instance v4, Ljava/util/ArrayList;

    invoke-direct {v4}, Ljava/util/ArrayList;-><init>()V

    .line 5
    new-instance v2, Ljava/util/ArrayList;

    invoke-direct {v2}, Ljava/util/ArrayList;-><init>()V

    .line 6
    new-instance v3, Landroidx/recyclerview/widget/DiffUtil$Range;

    const/4 v5, 0x0

    invoke-direct {v3, v5, v0, v5, v1}, Landroidx/recyclerview/widget/DiffUtil$Range;-><init>(IIII)V

    invoke-virtual {v2, v3}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    add-int/2addr v0, v1

    const/4 v1, 0x1

    add-int/2addr v0, v1

    .line 7
    div-int/lit8 v0, v0, 0x2

    .line 8
    new-instance v3, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;

    mul-int/lit8 v0, v0, 0x2

    add-int/2addr v0, v1

    invoke-direct {v3, v0}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;-><init>(I)V

    .line 9
    new-instance v6, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;

    invoke-direct {v6, v0}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;-><init>(I)V

    .line 10
    new-instance v0, Ljava/util/ArrayList;

    invoke-direct {v0}, Ljava/util/ArrayList;-><init>()V

    .line 11
    :goto_0
    invoke-virtual {v2}, Ljava/util/ArrayList;->isEmpty()Z

    move-result v7

    if-nez v7, :cond_8

    .line 12
    invoke-virtual {v2}, Ljava/util/ArrayList;->size()I

    move-result v7

    sub-int/2addr v7, v1

    invoke-virtual {v2, v7}, Ljava/util/ArrayList;->remove(I)Ljava/lang/Object;

    move-result-object v7

    check-cast v7, Landroidx/recyclerview/widget/DiffUtil$Range;

    .line 13
    invoke-static {v7, p0, v3, v6}, Landroidx/recyclerview/widget/DiffUtil;->midPoint(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;)Landroidx/recyclerview/widget/DiffUtil$Snake;

    move-result-object v8

    if-eqz v8, :cond_7

    .line 14
    invoke-virtual {v8}, Landroidx/recyclerview/widget/DiffUtil$Snake;->diagonalSize()I

    move-result v9

    if-lez v9, :cond_5

    .line 15
    iget v9, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->endY:I

    iget v10, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->startY:I

    sub-int/2addr v9, v10

    iget v11, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->endX:I

    iget v12, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->startX:I

    sub-int/2addr v11, v12

    if-eq v9, v11, :cond_0

    const/4 v13, 0x1

    goto :goto_1

    :cond_0
    const/4 v13, 0x0

    :goto_1
    if-eqz v13, :cond_4

    .line 16
    iget-boolean v13, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->reverse:Z

    if-eqz v13, :cond_1

    .line 17
    new-instance v9, Landroidx/recyclerview/widget/DiffUtil$Diagonal;

    invoke-virtual {v8}, Landroidx/recyclerview/widget/DiffUtil$Snake;->diagonalSize()I

    move-result v11

    invoke-direct {v9, v12, v10, v11}, Landroidx/recyclerview/widget/DiffUtil$Diagonal;-><init>(III)V

    goto :goto_3

    :cond_1
    if-le v9, v11, :cond_2

    const/4 v9, 0x1

    goto :goto_2

    :cond_2
    const/4 v9, 0x0

    :goto_2
    if-eqz v9, :cond_3

    .line 18
    new-instance v9, Landroidx/recyclerview/widget/DiffUtil$Diagonal;

    add-int/lit8 v10, v10, 0x1

    invoke-virtual {v8}, Landroidx/recyclerview/widget/DiffUtil$Snake;->diagonalSize()I

    move-result v11

    invoke-direct {v9, v12, v10, v11}, Landroidx/recyclerview/widget/DiffUtil$Diagonal;-><init>(III)V

    goto :goto_3

    .line 19
    :cond_3
    new-instance v9, Landroidx/recyclerview/widget/DiffUtil$Diagonal;

    add-int/lit8 v12, v12, 0x1

    invoke-virtual {v8}, Landroidx/recyclerview/widget/DiffUtil$Snake;->diagonalSize()I

    move-result v11

    invoke-direct {v9, v12, v10, v11}, Landroidx/recyclerview/widget/DiffUtil$Diagonal;-><init>(III)V

    goto :goto_3

    .line 20
    :cond_4
    new-instance v9, Landroidx/recyclerview/widget/DiffUtil$Diagonal;

    invoke-direct {v9, v12, v10, v11}, Landroidx/recyclerview/widget/DiffUtil$Diagonal;-><init>(III)V

    .line 21
    :goto_3
    invoke-virtual {v4, v9}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    .line 22
    :cond_5
    invoke-virtual {v0}, Ljava/util/ArrayList;->isEmpty()Z

    move-result v9

    if-eqz v9, :cond_6

    new-instance v9, Landroidx/recyclerview/widget/DiffUtil$Range;

    invoke-direct {v9}, Landroidx/recyclerview/widget/DiffUtil$Range;-><init>()V

    goto :goto_4

    .line 23
    :cond_6
    invoke-virtual {v0}, Ljava/util/ArrayList;->size()I

    move-result v9

    sub-int/2addr v9, v1

    .line 24
    invoke-virtual {v0, v9}, Ljava/util/ArrayList;->remove(I)Ljava/lang/Object;

    move-result-object v9

    check-cast v9, Landroidx/recyclerview/widget/DiffUtil$Range;

    .line 25
    :goto_4
    iget v10, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    iput v10, v9, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    .line 26
    iget v10, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    iput v10, v9, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    .line 27
    iget v10, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->startX:I

    iput v10, v9, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    .line 28
    iget v10, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->startY:I

    iput v10, v9, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    .line 29
    invoke-virtual {v2, v9}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    .line 30
    iget v9, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    iput v9, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    .line 31
    iget v9, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iput v9, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    .line 32
    iget v9, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->endX:I

    iput v9, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    .line 33
    iget v8, v8, Landroidx/recyclerview/widget/DiffUtil$Snake;->endY:I

    iput v8, v7, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    .line 34
    invoke-virtual {v2, v7}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto/16 :goto_0

    .line 35
    :cond_7
    invoke-virtual {v0, v7}, Ljava/util/ArrayList;->add(Ljava/lang/Object;)Z

    goto/16 :goto_0

    .line 36
    :cond_8
    sget-object v0, Landroidx/recyclerview/widget/DiffUtil;->DIAGONAL_COMPARATOR:Ljava/util/Comparator;

    invoke-static {v4, v0}, Ljava/util/Collections;->sort(Ljava/util/List;Ljava/util/Comparator;)V

    .line 37
    new-instance v0, Landroidx/recyclerview/widget/DiffUtil$DiffResult;

    .line 38
    invoke-virtual {v3}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->backingData()[I

    move-result-object v5

    invoke-virtual {v6}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->backingData()[I

    move-result-object v6

    move-object v2, v0

    move-object v3, p0

    move v7, p1

    invoke-direct/range {v2 .. v7}, Landroidx/recyclerview/widget/DiffUtil$DiffResult;-><init>(Landroidx/recyclerview/widget/DiffUtil$Callback;Ljava/util/List;[I[IZ)V

    return-object v0
.end method

.method private static forward(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;I)Landroidx/recyclerview/widget/DiffUtil$Snake;
    .locals 11

    .line 1
    iget v0, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    iget v1, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    sub-int/2addr v0, v1

    .line 2
    iget v1, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iget v2, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    sub-int/2addr v1, v2

    sub-int/2addr v0, v1

    .line 3
    invoke-static {v0}, Ljava/lang/Math;->abs(I)I

    move-result v0

    rem-int/lit8 v0, v0, 0x2

    const/4 v1, 0x0

    const/4 v2, 0x1

    if-ne v0, v2, :cond_0

    goto :goto_0

    :cond_0
    const/4 v2, 0x0

    .line 4
    :goto_0
    iget v0, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    iget v3, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    sub-int/2addr v0, v3

    .line 5
    iget v3, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iget v4, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    sub-int/2addr v3, v4

    sub-int/2addr v0, v3

    neg-int v3, p4

    move v4, v3

    :goto_1
    if-gt v4, p4, :cond_7

    if-eq v4, v3, :cond_2

    if-eq v4, p4, :cond_1

    add-int/lit8 v5, v4, 0x1

    .line 6
    invoke-virtual {p2, v5}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v5

    add-int/lit8 v6, v4, -0x1

    invoke-virtual {p2, v6}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v6

    if-le v5, v6, :cond_1

    goto :goto_2

    :cond_1
    add-int/lit8 v5, v4, -0x1

    .line 7
    invoke-virtual {p2, v5}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v5

    add-int/lit8 v6, v5, 0x1

    goto :goto_3

    :cond_2
    :goto_2
    add-int/lit8 v5, v4, 0x1

    .line 8
    invoke-virtual {p2, v5}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v5

    move v6, v5

    .line 9
    :goto_3
    iget v7, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    iget v8, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    sub-int v8, v6, v8

    add-int/2addr v8, v7

    sub-int/2addr v8, v4

    if-eqz p4, :cond_4

    if-eq v6, v5, :cond_3

    goto :goto_4

    :cond_3
    add-int/lit8 v7, v8, -0x1

    goto :goto_5

    :cond_4
    :goto_4
    move v7, v8

    .line 10
    :goto_5
    iget v9, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    if-ge v6, v9, :cond_5

    iget v9, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    if-ge v8, v9, :cond_5

    .line 11
    invoke-virtual {p1, v6, v8}, Landroidx/recyclerview/widget/DiffUtil$Callback;->areItemsTheSame(II)Z

    move-result v9

    if-eqz v9, :cond_5

    add-int/lit8 v6, v6, 0x1

    add-int/lit8 v8, v8, 0x1

    goto :goto_5

    .line 12
    :cond_5
    invoke-virtual {p2, v4, v6}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->set(II)V

    if-eqz v2, :cond_6

    sub-int v9, v0, v4

    add-int/lit8 v10, v3, 0x1

    if-lt v9, v10, :cond_6

    add-int/lit8 v10, p4, -0x1

    if-gt v9, v10, :cond_6

    .line 13
    invoke-virtual {p3, v9}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->get(I)I

    move-result v9

    if-gt v9, v6, :cond_6

    .line 14
    new-instance p0, Landroidx/recyclerview/widget/DiffUtil$Snake;

    invoke-direct {p0}, Landroidx/recyclerview/widget/DiffUtil$Snake;-><init>()V

    .line 15
    iput v5, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->startX:I

    .line 16
    iput v7, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->startY:I

    .line 17
    iput v6, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->endX:I

    .line 18
    iput v8, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->endY:I

    .line 19
    iput-boolean v1, p0, Landroidx/recyclerview/widget/DiffUtil$Snake;->reverse:Z

    return-object p0

    :cond_6
    add-int/lit8 v4, v4, 0x2

    goto :goto_1

    :cond_7
    const/4 p0, 0x0

    return-object p0
.end method

.method private static midPoint(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;)Landroidx/recyclerview/widget/DiffUtil$Snake;
    .locals 7

    .line 1
    iget v0, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    iget v1, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListStart:I

    sub-int v2, v0, v1

    const/4 v3, 0x0

    const/4 v4, 0x1

    if-lt v2, v4, :cond_3

    .line 2
    iget v2, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListEnd:I

    iget v5, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->newListStart:I

    sub-int v6, v2, v5

    if-ge v6, v4, :cond_0

    goto :goto_1

    :cond_0
    sub-int/2addr v0, v1

    sub-int/2addr v2, v5

    add-int/2addr v2, v0

    add-int/2addr v2, v4

    .line 3
    div-int/lit8 v2, v2, 0x2

    .line 4
    invoke-virtual {p2, v4, v1}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->set(II)V

    .line 5
    iget v0, p0, Landroidx/recyclerview/widget/DiffUtil$Range;->oldListEnd:I

    invoke-virtual {p3, v4, v0}, Landroidx/recyclerview/widget/DiffUtil$CenteredArray;->set(II)V

    const/4 v0, 0x0

    :goto_0
    if-ge v0, v2, :cond_3

    .line 6
    invoke-static {p0, p1, p2, p3, v0}, Landroidx/recyclerview/widget/DiffUtil;->forward(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;I)Landroidx/recyclerview/widget/DiffUtil$Snake;

    move-result-object v1

    if-eqz v1, :cond_1

    return-object v1

    .line 7
    :cond_1
    invoke-static {p0, p1, p2, p3, v0}, Landroidx/recyclerview/widget/DiffUtil;->backward(Landroidx/recyclerview/widget/DiffUtil$Range;Landroidx/recyclerview/widget/DiffUtil$Callback;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;Landroidx/recyclerview/widget/DiffUtil$CenteredArray;I)Landroidx/recyclerview/widget/DiffUtil$Snake;

    move-result-object v1

    if-eqz v1, :cond_2

    return-object v1

    :cond_2
    add-int/lit8 v0, v0, 0x1

    goto :goto_0

    :cond_3
    :goto_1
    return-object v3
.end method
